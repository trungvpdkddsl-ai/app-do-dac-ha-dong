...
export const GasSettings: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    setGasUrl(url);
    setSaved(true);
    showToast('📌 Đã lưu kết nối thành công. Dữ liệu sẽ không bị mất khi tải lại trang.');
    setMessage('✅ Đã lưu kết nối. Đang tải lại dữ liệu từ server mới...');

    setIsReloading(true);
    try {
      await reloadData();
      setMessage('✅ Đã lưu kết nối và tải lại dữ liệu thành công!');
    } catch {
      setMessage('✅ Đã lưu kết nối. Không thể tải lại ngay — sẽ đồng bộ lần sau.');
    } finally {
    ...
    <div className="p-6 max-w-3xl mx-auto relative">
        ...
        <div className="flex items-start gap-3 bg-emerald-50 border border-emerald-200 rounded-2xl p-4 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-600 mt-0.5">
              URL đang được lấy từ biến môi trường <code className="bg-emerald-100 px-1 rounded font-mono">VITE_GAS_URL</code>.
            </p>
          </div>
        </div>
      )}
      ...
      {!isEnvMode && localStorage.getItem('GAS_URL') && (
        <div className="flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-2xl p-4 mb-4">
          ...
          <div>
            <p className="text-sm font-semibold text-blue-800">Đang dùng kết nối đã lưu</p>
            <p className="text-xs text-blue-600 mt-0.5">
              URL được lưu trong trình duyệt này. Dữ liệu sẽ giữ nguyên khi tải lại trang.
            </p>
          </div>
        </div>
      )}
      ...
      <div className={`bg-white rounded-2xl border p-6 mb-4 transition-opacity ${isEnvMode ? 'border-slate-200 opacity-60' : 'border-slate-200'}`}>
        ...
        <p className="text-xs text-slate-500 mb-4">
          {isEnvMode
            ? 'URL bên dưới chỉ để xem.'
            : 'Dán URL GAS của bạn vào đây và nhấn "Lưu kết nối" để ứng dụng kết nối với Database của bạn.'}
        </p>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
        ...
        <ol className="text-sm text-amber-700 space-y-2">
          ...
          <li><strong>5.</strong> Copy URL mới → dán vào ô trên → Lưu kết nối</li>
        </ol>
